...
export async function GET(req) {
  ...
  if (!PEXELS_API_KEY) {
    ...
    return new Response(
      JSON.stringify({
        error: "PEXELS_API_KEY is not set in environment variables.",
      }),
      {
        status: 500,
  ...
  try {
    ...
    if (!pexelsResponse.ok) {
      ...
      return new Response(
        JSON.stringify({
          error: `Failed to fetch images from Pexels: ${pexelsResponse.statusText}. Raw error: ${errorText}`,
        }),
        {
    ...
    if (!data.photos || !Array.isArray(data.photos)) {
      ...
      return new Response(
        JSON.stringify({
          error: "Invalid Pexels API response structure: missing photos array.",
        }),
        {
          status: 500,
          headers: { "Content-Type": "application/json" },
    ...
    return new Response(
      JSON.stringify({
        error:
          error.message || "An unknown error occurred while fetching images.",
      }),
      {
        status: 500,
        headers: { "Content-Type": "application/json" },
