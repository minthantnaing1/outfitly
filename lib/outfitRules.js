...
export function getAllRecommendations(
  ...
  const isLiked = (x) => {
    ...
    const g = globCounts.get(x.key) || { likes: 0, dislikes: 0 };
    // any effective (decayed) like in context OR globally
    return c.likes > 0.0001 || g.likes > 0.0001;
  };

  const likedFirst = filtered.filter(isLiked).sort((a, b) => b.score - a.score);
  const remaining = filtered.filter((x) => !isLiked(x));
