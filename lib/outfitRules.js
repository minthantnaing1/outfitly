...
export function getAllRecommendations(
  ...
  //
  // ... (old exploration + finalOrder) ...
  //
  // return finalOrder.map(...)

  // --- NEW: pin liked items first, then apply MMR to the remaining ---
  // Pin ONLY items liked in THIS CONTEXT (same weather/occasion/mood)
  const likedInContext = (x) => {
    const c = ctxCounts.get(x.key) || { likes: 0, dislikes: 0 };
    return c.likes > 0.0001; // decayed likes in current context only
  };

  const likedFirst = filtered
    .filter(likedInContext)
    .sort((a, b) => b.score - a.score);
  const remaining = filtered.filter((x) => !likedInContext(x));

  // Fill as many top slots with liked items as possible (up to K)
  const likedTake = Math.min(K, likedFirst.length);
  const likedTop = likedFirst.slice(0, likedTake);

  // Use MMR for the remaining slots
  const mmrSlots = Math.max(0, K - likedTake);
  const { picked: mmrPicked, rest } = rerankMMR(remaining, mmrSlots, mmrLambda);

  // Optional exploration: only affects the non-liked MMR portion
  if (exploreTau > 0 && rest.length > 0 && mmrPicked.length > 0) {
    const exploratory = softmaxPick(rest, exploreTau);
    if (exploratory) {
      const exists = mmrPicked.some((p) => p.key === exploratory.key);
      if (!exists) mmrPicked[mmrPicked.length - 1] = exploratory;
    }
  }

  const topSegment = [...likedTop, ...mmrPicked];
  const finalOrder = [...topSegment, ...rest];
